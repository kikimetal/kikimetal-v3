...
// UPDATE: 2018.11.11 クエリストリングにGSSID も含めるように汎用化 -> kikimetal.com からのアクセスのため
...
if (!empty($_GET) && !empty($_GET["sheetName"])) {
  ...
  $id = '';
  if (empty($_GET["GSSID"])) {
    $id = '1QUfjuyezpUIzpLWoTyrq52OPdK8Gkfj_UNNqoui_964';
  } else {
    $id = $_GET["GSSID"];
  }
  $sheet_data_json_obj = getJSONfromGSS($_GET["sheetName"], $_GET, $id);
